...
void DS1302_GPIOInit(void)
{
  ...
  GPIO_InitStructre.GPIO_Pin= GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9;
...
void DS1302_IO_GPIO(uchar FLAG)
{
  ...
  //RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);            /*open GPIO  clock*/
    ...
    if(FLAG==0x01)
  {
    GPIO_InitStructre.GPIO_Pin= GPIO_Pin_8;//配置IO_GPIO
  GPIO_InitStructre.GPIO_Speed=GPIO_Speed_50MHz;
  GPIO_InitStructre.GPIO_Mode=GPIO_Mode_Out_PP;
  GPIO_Init(DS1302_PORT, &GPIO_InitStructre);
  }
...
void display_year(uchar a,uchar flag)//flag为1 时，显示数据，0时对应清除屏幕
{
	if(flag)
	{
	OLED_ShowChar(24, 0, a/10+16+'0');//年	
	OLED_ShowChar(32, 0, a%10+16+'0');
	}
	else
	{
	//清除年位
...
void display_month(uchar a,uchar flag)
{
	if(flag)
	{
	OLED_ShowChar(56, 0, a/10+16+'0');//月
	OLED_ShowChar(64, 0, a%10+16+'0');
	}
	else
	{
	//清除月位
...
void display_day(uchar a,uchar flag)
{
	if(flag)
	{
	OLED_ShowChar(88, 0, a/10+16+'0');//日
	OLED_ShowChar(96, 0, a%10+16+'0');
	}
	else
	{
	//清除日位
...
void show_time(uchar show_data[6])//时间打印函数
{
	...
	OLED_ShowChar(24, 0, show_data[0]/10+16+'0');//年	
	OLED_ShowChar(32, 0, show_data[0]%10+16+'0');
	
	OLED_ShowChar(56, 0, show_data[1]/10+16+'0');//月
	OLED_ShowChar(64, 0, show_data[1]%10+16+'0');
	
	OLED_ShowChar(88, 0, show_data[2]/10+16+'0');//日
	OLED_ShowChar(96, 0, show_data[2]%10+16+'0');
