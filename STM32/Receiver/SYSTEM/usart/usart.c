...
u8 i;
u8 buffdata1[9]={0xFE,0x05,0xAA,0xAA,0x9A,0x8A,0,0xFF,0xF1}; //发送状态
u8 buffdata2[9]={0xFE,0x05,0xBB,0xBB,0x9A,0x8A,0,0xFF,0xF1}; //发送状态
u8 buffdata3[9]={0xFE,0x05,0xCC,0xCC,0x9A,0x8A,0,0xFF,0xF1}; //发送状态
u8 buffdata4[9]={0xFE,0x05,0xDD,0xDD,0x9A,0x8A,0,0xFF,0xF1}; //发送状态
u8 buffdata5[9]={0xFE,0x05,0xEE,0xEE,0x9A,0x8A,0,0xFF,0xF1}; //发送状态
...
void USART1_IRQHandler(void)
{
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) //接收中断
	{
		...
		if(usart1_rx_buf[0]=='A')
			...
			{
				while(buffdata2[i]!=0xF1)  // 每个字符串结尾 都是以   结尾的
				{
			...
			{
				while(buffdata5[i]!=0xF1)  // 每个字符串结尾 都是以   结尾的
				{
					USART_SendData(USART2,buffdata5[i]);      //通过库函数  发送数据
					while( USART_GetFlagStatus(USART2,USART_FLAG_TC)!= SET);  
					//等待发送完成。  检测 USART_FLAG_TC 是否置1；   //见库函数 P359 介绍
					i++;              //i++一次
				}
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		{
			...
			{
				 if(usart2_rx_buf[0] == 0xFE&&usart2_rx_buf[2] == 0xA0)                      //检测到头标识是我们需要的 
				 {
					 temp=usart2_rx_buf[6];
					 OLED_ShowNum(40,0, temp, 2, 1);
					 usart2_rx_index=0;                                   
				 }
				else if(usart2_rx_buf[0] == 0xFE&&usart2_rx_buf[2] == 0xA1)                      //检测到头标识是我们需要的 
				 {
					 humi=usart2_rx_buf[6];
					 OLED_ShowNum(104,0, humi, 2, 1);
					usart2_rx_index=0;                                   
				 }
				 else if(usart2_rx_buf[0] == 0xFE&&usart2_rx_buf[2] == 0xA2)                      //检测到头标识是我们需要的 
				 {
					 mq2=usart2_rx_buf[6];
					 OLED_ShowNum(40,2, mq2, 2, 1);
					usart2_rx_index=0;                                   
				 }
				  else if(usart2_rx_buf[0] == 0xFE&&usart2_rx_buf[2] == 0xA3)                      //检测到头标识是我们需要的 
				 {
					 light=usart2_rx_buf[6];
					 OLED_ShowNum(104,2, light, 2, 1);
//					 OLED_ShowNum(40,4, mq2th, 2, 1);
					usart2_rx_index=0;                                   
				 }
				 else
				 {
